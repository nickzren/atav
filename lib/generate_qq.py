...
def read_summary_file(summary_file):
    ...
    with open(summary_file, 'r') as infile:
        ...
        max_qual = int(first[qcase_index]) + int(first[qctrl_index])
        # read all remaining lines and add info to gene dict 
        for line in infile:
            line = line.strip().split(',')
            genes[line[gene_index]] = [x for x in line[2:]]
            if int(line[qcase_index]) + int(line[qctrl_index]) > max_qual:
                max_qual = int(line[qcase_index]) + int(line[qctrl_index])
    return (genes, ncase, nctrl, max_qual)
...
if __name__ == '__main__':
    ...
    genes, ncase, nctrl, max_qual = read_summary_file(args.partial_summary_file)
    ...
    max_case = min([max_qual, ncase])
    max_ctrl = min([max_qual, nctrl])
    ...
    pairs = []
    # if lookup matrix is square, only calculate half of it
    if max_case == max_qual and max_ctrl == max_qual:
        for i in xrange(max_case + 1):
            for j in xrange(max_ctrl + 1 - i):
                pairs.append((i,j))
    # otherwise it's easiest to just calculate the whole thing
    else:
        for i in xrange(max_case + 1):
            for j in xrange(max_ctrl):
                pairs.append((i, j))
    ...
    for gene, v in genes.iteritems():
        v.append(lookup[int(v[3]),int(v[6])])
    ordered = sorted(genes.items(), key=lambda x:x[1][-1])

    # create sorted gene name list
