...
data   <- data[is.na(data[normalized.name(paste0("Percent Read Alt (child) (#",dnum,")"))])
             | data[normalized.name(paste0("Percent Read Alt (child) (#",dnum,")"))] >= 0.2
             | (data[normalized.name(paste0("Pass Fail Status (child) (#",dnum,")"))] == "pass"
             & (data[normalized.name(paste0("Variant Type (#",dnum,")"))] == "snv"
               |data[normalized.name(paste0("Het Binomial P (child) (#",dnum,")"))] > 0.01)),]
...
data   <- data[is.na(data[normalized.name("Percent Read Alt (child)")])
             ...
             | (data[normalized.name("Pass Fail Status (child)")] == "pass"
             &  (data[normalized.name("Variant Type")] == "snv"
               | data[normalized.name("Het Binomial P (child)")] > 0.01)),]
if (dim(data)[1] ==0) { return(data)}
