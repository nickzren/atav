...
def save(data, name):
    ...
    if not outputS3:
        Popen('hadoop fs -getmerge ' + outputFile + ' ' + outputFile +
              '.csv && hadoop fs -rm -f -r ' + outputFile, shell=True)
...
sqlContext.registerFunction(
    "date_in_interval", date_in_interval, BooleanType())
...
timeframes = ["[0,10min[", "[10min,1h[", "[1h,6h[",
              "[6h,12h[", "[12h,1d[", "[1d,2d[", "[2d+"]
...
# functionCountData[<interval-id>][<function-name>] has the number of
# times such function was used in such interval
...
for inx, interval in enum(intervals):
    ...
    functionUsagePairs = r.map(lambda l: (
        l.fun, 1)).reduceByKey(lambda x, y: x + y)

    functionCountData.append(functionUsagePairs.collectAsMap())

    newRows = functionUsagePairs.map(lambda t: Row(
        series=intervalstr, function=t[0], value=t[1])).collect()
    functionUsageData.extend(newRows)

    # Timeframe Analysis
    funTimeframePairs = r.map(lambda l: ((l.fun, get_timeframe(l.time), get_user_group(
        l.user)), (1, l.time))).reduceByKey(lambda x, y: (x[0] + y[0], x[1] + y[1]))
    ...
    # CPU Time Analysis
    cpuTimePairs = r.map(lambda l: (get_user_group(
        l.user), l.time)).reduceByKey(lambda x, y: x + y)
