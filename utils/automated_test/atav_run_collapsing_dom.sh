...
atav_latest.sh --disable-timestamp-from-out-path --sample $ATAV_TEST_PATH/trio_sample.txt --effect $ATAV_TEST_PATH/function --region $ATAV_TEST_PATH/region --out $ATAV_OUTPUT_PATH/old/collapsing_dom --collapsing-dom --min-coverage 10 --filter PASS --ccds-only --evs-maf 0.1 --loo-af 0.01
