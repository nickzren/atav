...
atav_latest.sh --disable-timestamp-from-out-path --sample $ATAV_TEST_PATH/trio_sample.txt --effect $ATAV_TEST_PATH/function --region $ATAV_TEST_PATH/region --out $ATAV_OUTPUT_PATH/old/collapsing_rec --collapsing-rec --min-coverage 10 --filter PASS --ccds-only --evs-maf 0.10 --loo-af 0.01
