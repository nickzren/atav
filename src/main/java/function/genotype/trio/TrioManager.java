...
public class TrioManager {
    ...
    private static final String DENOVO_RULES_PATH = "data/trio_rule_112216.txt";
    ...
    private static String getTitleByVariant() {
        return "Denovo Flag,"
                ...
                + "Het Binomial P (child),"
                + "Hom Binomial P (child),"
    ...
    private static void initDenovoRules() {
        ...
        if (!RuleFile.exists()) {
            ErrorManager.print("The trio rule file (" + trioRulesPath + ") doesn't exist.", ErrorManager.UNEXPECTED_FAIL);
        }
        try {
            ...
            while ((str = br.readLine()) != null) {
                if (LineCount > 0) {
                    ...
                    if (fields.length != 10) {
                        ErrorManager.print("Wrong trio rule file format.", ErrorManager.UNEXPECTED_FAIL);
                    }
                    ...
                    if (StatusKey.contains("?")) { //un recognized field value
                        ErrorManager.print(sKey.append(": parsing trio rule failed at Line ").append(LineCount + 1).toString(), ErrorManager.UNEXPECTED_FAIL);
                    }
