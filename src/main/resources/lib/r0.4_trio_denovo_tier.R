...
data <- Filter.by.DenovoFlag(atav.output,'DE NOVO', strict = TRUE)
data <- Filter.for.denovo(data)
write.csv(data, file = paste(atavOutput,"/denovo.csv", sep=""))

data <- Filter.by.DenovoFlag(atav.output,'NEWLY HEMIZYGOUS', strict = TRUE)
data <- Filter.for.hemizygous(data)
write.csv(data, file = paste(atavOutput,"/hemizygous.csv", sep=""))

data <- Filter.by.DenovoFlag(atav.output,'NEWLY HOMOZYGOUS', strict = TRUE)
