do_denovo_tier <- function(denovoFile,atavOutput){
...
write.csv(data, file = paste(atavOutput,"/denovo_tier2.csv"))

data <- Filter.by.Flag(input.data,'newly hemizygous', strict = FALSE)
data <- Filter.by.HemiHomo.Count(data,9)
data <- Filter.for.tier2(data)
write.csv(data, file = paste(atavOutput,"/hemizygous_tier2.csv"))

data <- Filter.by.Flag(input.data,'newly homozygous', strict = FALSE)
data <- Filter.by.HemiHomo.Count(data,9)
data <- Filter.for.tier2(data)
write.csv(data, file = paste(atavOutput,"/homozygous_tier2.csv"))
}
### MAIN ###
args <- commandArgs(T)
